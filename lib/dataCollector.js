...
async function getPostDetails(postUrl) {
  try {
    ...
    if (contentHtml) {
      ...
      // 🔥 1. SE3.0 이미지 컴포넌트 카운트 (가장 정확)
      const seImageComponents = contentHtml.match(/<div[^>]*class="[^"]*se-component[^"]*se-image-resource[^"]*"/gi) || [];
      console.log(`📊 SE3.0 이미지 컴포넌트: ${seImageComponents.length}개 발견`);
      
      // 🔥 2. SE3.0 이미지 모듈 카운트
      const seImageModules = contentHtml.match(/<div[^>]*class="[^"]*se-module[^"]*se-module-image[^"]*"/gi) || [];
      console.log(`📊 SE3.0 이미지 모듈: ${seImageModules.length}개 발견`);
      
      // SE3.0 방식으로 카운트 (중복 제거)
      const se3Count = Math.max(seImageComponents.length, seImageModules.length);
      if (se3Count > 0) {
        imageCount = se3Count;
        console.log(`✅ SE3.0 방식으로 이미지 ${imageCount}개 카운트됨`);
      }
      ...
      // 4. <se-image> 태그 (커스텀 태그)
      const seImageTags = contentHtml.matchAll(/<se-image[^>]*(?:data-src|src)="([^"]+)"/gi);
      let seImageTagCount = 0;
      for (const match of seImageTags) {
        imageUrls.add(match[1]);
        seImageTagCount++;
      }
      console.log(`📊 <se-image> 태그: ${seImageTagCount}개 발견`);
      ...
      // 최종 카운트: SE3.0 카운트가 있으면 우선, 없으면 img 태그 카운트
      if (imageCount === 0) {
        imageCount = imageUrls.size;
      }
      
      console.log(`✅ 본문 이미지 개수 (최종): ${imageCount}개`);
      
      // 🔍 디버깅: 실제 이미지 URL 샘플 출력
      if (imageUrls.size > 0) {
        const sampleUrls = Array.from(imageUrls).slice(0, 3);
        console.log(`📸 이미지 URL 샘플 (최대 3개):`);
        sampleUrls.forEach((url, idx) => {
          console.log(`   ${idx + 1}. ${url.substring(0, 80)}...`);
        });
      }
      
      // 디버깅: SE3.0 이미지 컴포넌트 샘플 출력
      if (seImageComponents.length > 0 && imageCount === 0) {
        console.warn(`⚠️ SE3.0 컴포넌트는 있는데 이미지 카운트가 0! HTML 샘플:`);
        const imageComponentMatch = contentHtml.match(/<div[^>]*class="[^"]*se-component[^"]*se-image-resource[^"]*"[^>]*>[\s\S]{0,500}/i);
        if (imageComponentMatch) {
          console.log(imageComponentMatch[0]);
        }
      }
