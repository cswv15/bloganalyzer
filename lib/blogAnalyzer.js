...
const { analyzeIndividualPost, generateImprovementSuggestions } = require('./postAnalyzer');
...
async function analyzeBlog(blogUrl) {
  try {
    ...
    // 1단계: 기본 데이터 수집
    console.log(`\n[1단계] 기본 데이터 수집 중...\n`);
    const totalPosts = await getTotalPostCount(blogId) || 10;
    ...
    // 3.5단계: 개별 글 분석 (신규 추가)
    console.log(`\n[3.5단계] 개별 게시물 점수 계산 중...\n`);
    const analyzedPosts = detailedPosts.map(post => {
      const analysis = analyzeIndividualPost(post);
      const suggestions = generateImprovementSuggestions(post, analysis);
      console.log(`  - ${post.title.substring(0, 30)}: ${analysis.totalScore}점`);
      return {
        ...post,
        analysis,
        suggestions,
    ...
    console.log(`   - 누적 방문자: ${Math.floor(avgViews * totalPosts)}명`);
    console.log(`   - 평균 댓글: ${Math.round(avgComments)}개`);
    console.log(`   - 평균 공감: ${Math.round(avgLikes)}개`);
    console.log(`   - 평균 조회수: ${Math.round(avgViews)}회`);
    console.log(`========================================\n`);
    ...
    return {
      ...
      rank: estimatedRank,
      category: '일반',
      recentPosts: analyzedPosts.slice(0, 10).map(p => ({
        title: p.title,
        link: p.link,
        postdate: p.postdate,
        ...
        influenceScore: p.analysis.influenceScore,
        contentScore: p.analysis.contentScore,
        socialScore: p.analysis.socialScore,
